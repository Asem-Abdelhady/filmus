import {
  ...
  const Movies = () => {
    ...
    const fetchTrending = async () => {
      const { data } = await axios.get(
        `${base_url}/discover/movie?api_key=a500ed6497632b594464be767b4d390d&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=${page}`
  ...
  for (var d in content) {
    list.push(<Card key={"card_Movie_" + d} 
      id={content[d].id}
